...
const KitchenDisplay = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Filtrar pedidos con estado pendiente o preparando
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      fetchOrders(); // refrescar lista después de actualizar
    } catch (error) {
    ...
    <div className="kitchen-display p-6 bg-white rounded-lg shadow-md">
      ...
      {orders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {orders.map(order => (
            <div key={order.id} className="order-card p-4 border rounded shadow">
